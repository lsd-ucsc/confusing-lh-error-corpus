...
{-@ halve :: i:Int -> (Int, Int)<{\j1 j2 -> j1 == div i 2 && j2 == j1 + (i mod 2)}> @-}
halve :: Int -> (Int, Int)
halve i = (j, j + r)
  where
    j = i `div` 2
...
{-@ sublist :: s:Nat -> l:Nat -> {xs:List a | s <= length xs && l <= (length xs - s)} -> {ys:List a | length ys == l} @-}
...
{-@ snoc :: xs:List a -> x:a -> {ys:List a | length ys == length xs + 1} @-}
